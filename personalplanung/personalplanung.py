...
# month_year = st.text_input("Monat und Jahr (z.B. 'August 2023'):", help='Geben Sie den Monat und das Jahr im Format "Monat Jahr" ein.',value=datetime.now().strftime("%B %Y"))
# employees = st.text_input("Mitarbeiterliste (getrennt durch Kommas):", help='Geben Sie die Namen der Mitarbeiter ein, getrennt durch Kommas.',value="Alina,Pauline,Steffi,Luca,Flori,Annina")
# weekday_to_number = {
#     'Montag': 1,
#     'Dienstag': 2,
#     'Mittwoch': 3,
...
# rest_days = st.multiselect('Ruhetage:', list(weekday_to_number.keys()), help='Wählen Sie die Ruhetage aus.',default=["Dienstag","Mittwoch"])
...
#             st.write(employee)
#         with col2:
#             work_hours[employee] = st.text_input(f"Arbeitszeit für {employee}", help="Geben Sie die typischen Arbeitszeiten für diesen Mitarbeiter ein. \nFormat: 9-17",value="9-17")
#         with col3:
#             holidays[employee] = st.text_input(f"Urlaubstage für {employee}", help="Format in Tagen des Monats: 1-9,21")
...
#     df.replace(np.nan, '', regex=True, inplace=True) # Replaces NaN values with an empty string
#     df.index = df.index.strftime('%d, %A')

#     #df.index = [month_year] + df.index.tolist()[1:]
#     st.dataframe(df)
...
# Erstellen Sie die Eingabefelder
month_year = st.selectbox("Monat und Jahr (z.B. 'August 2023'):", options=list(months_to_number.keys()), help='Wählen Sie den Monat aus der Liste aus.',index=7)
